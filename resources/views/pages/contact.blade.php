        ...
        <div class="col-md-7">
            ...
            <form method="POST" action="{{ route('contact.store') }}" style="background:var(--white);border:1px solid var(--border-light);border-radius:var(--radius-md);padding:2rem;box-shadow:var(--shadow-soft);">
                ...
                <div class="contact-form-group">
                    ...
                    <input type="text" id="subject" name="subject" value="{{ old('subject') }}" required placeholder="Objet de votre message">
                    @error('subject')<p class="error-text">{{ $message }}</p>@enderror
                </div>

                <div class="contact-form-group">
                    <label for="message">Message</label>
                    <textarea id="message" name="message" required placeholder="Votre message…">{{ old('message') }}</textarea>
